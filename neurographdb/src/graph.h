...
public:
    ...
    /* 경로 빔서치 — 확산과 달리 **경로 구조를 유지한 채** 점수를 매긴다.
     *
     * 기존 spread()는 노드 활성을 퍼뜨리고 경로 최대값만 남긴다. 즉 어떤 경로로
     * 왔는지를 버린다. PropRAG의 절제 실험에 따르면 경로 단위 빔서치가 그들 이득의
     * 가장 큰 단일 기여였다(+2.7%p, 명제 표현은 +0.8%p). 그리고 LLM을 쓰지 않는다.
     *
     * qsim: 노드별 질의 유사도(길이 n_). 게이팅에서 이미 계산하는 값과 같다.
     *
     * score_mode — 경로를 어떻게 점수화할지. 셋 다 근거가 다르다:
     *   0 MEAN  경로 노드 qsim의 평균. 짧은 경로가 유리하지 않다.
     *           다만 **강한 경로에 얹힌 약한 노드가 과대평가**된다.
     *   1 MIN   경로의 최약 노드. "사슬은 가장 약한 고리만큼 강하다."
     *           중간에 엉뚱한 문단을 거친 경로를 걸러낸다.
     *   2 LAST  도착 노드의 qsim만. 경로는 도달 수단으로만 쓴다.
     *
     * **seed마다 따로 빔을 돌리고 합친다.** 하나의 빔에 seed를 다 넣으면 폭 제한에
     * 걸려 즉시 잘려나가고, 그러면 희소 seed의 다양성이 죽는다 — 우리 방법의 근간이다.
     *
     * 노드 최종 점수 = 그 노드로 끝나는 경로들 중 최고 점수. */
    std::vector<ScoredDoc> beam_search(const std::vector<int32_t>& seeds,
                                       const std::vector<float>&   qsim,
                                       int   max_depth   = 3,
                                       int   beam_width  = 4,
                                       float min_sim     = 0.0f,
                                       int   score_mode  = 0) const;
